...
//!
//! Distinct from [`hopf_core`]'s connection-level traffic tracker: this
//! crate is about how much storage (and, optionally, how many messages) a
//! *user* has accumulated — across however many connections or protocols
//! touch their data. The same [`QuotaManager`] can back an FTP file store
//! and an IMAP mailbox at once.
//!
//! Resolution priority, mirroring Gumdrop: user-specific quota (highest),
//! then role-based, then the system default, then unlimited.
...
/// A user's quota limits and current usage. Either limit may be
/// [`UNLIMITED`] (`-1`).
