...
- [ ] **DTLS 1.3 real-peer interop** — closes the gap Phase 6 explicitly left open (no RFC 9147 equivalent of RFC 8448's known-answer vectors, and no real peer was available when that phase was written). **wolfSSL** is the recommended peer — the first DTLS 1.3 implementation (shortly after RFC 9147 published), still under active RFC-compliance maintenance, and its example client/server support DTLS 1.3 via a CLI flag. Ruled out: GnuTLS (no DTLS 1.3 in any released version), mbedTLS (explicitly unsupported), s2n-tls (no DTLS support at all), NSS (only stale pre-RFC draft-era support), pion/dtls (DTLS 1.3 unreleased). BoringSSL has real DTLS 1.3 support but no stable release and no comparable CLI tool — disproportionate integration cost for the same coverage.
  - Same shape as this project's existing subprocess-over-real-UDP interop harness (`dtls12::interop_tests`, OpenSSL-based) — reuse that harness's structure, swapping the OpenSSL subprocess invocations for wolfSSL's compiled example client/server.
  - **Watch item, not this pass's dependency**: OpenSSL is expected to land a stable DTLS 1.3 release — worth adding as a *second* peer once stable, since the existing OpenSSL-based DTLS 1.2 harness would need only new flags, not new tooling.
  - **Tests:** mirror `dtls12::interop_tests`'s shape (real subprocess + real UDP socket, both directions) since wolfSSL has no Rust bindings usable in-process here. Expect this to surface real bugs the way every other real-interop pass this project has run has (Phase 4: 5 bugs; Phase 5: 1; Phase 6 DTLS 1.2: 5) — RFC 9147's wire shapes in this codebase are explicitly flagged as "cross-checked against the RFC text but not proven against a real peer" today.
