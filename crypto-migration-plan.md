...
- [ ] **OCSP stapling (RFC 6960 / RFC 6066 §8 `status_request` / RFC 8446 §4.4.2.1), downgraded from P0 to nice-to-have (2026-09-10) — deliberately not started this pass.** Applies to both TLS 1.2 and TLS 1.3. Scoped and explored (3 parallel Explore agents) before being shelved, so a future pass doesn't re-derive this: this codebase has zero OCSP code today, and `crypto::x509::ParsedCertificate` extracts no serial number, AIA, or Extended Key Usage; there's also no HTTP client anywhere in the workspace capable of fetching from a live OCSP responder (`hopf-http` is stream-first and needs a full async runtime; the one existing plaintext client is OTLP-specific). Two scope decisions were reasoned through and should carry forward: (1) the server should obtain its OCSP response from **deployer-supplied bytes** (`Option<Bytes>` config, refreshed externally — same shape as `ticket_key`), not live CA fetching, since real stapling infra normally re-fetches on a background timer, a fundamentally different (scheduled async I/O) component from these synchronous, no-I/O-of-their-own handshake engines; (2) client-side validation should only need to handle **directly issuer-signed** responses, not delegated OCSP-responder certificates (RFC 6960 §4.2.2.2 — common with real CAs, e.g. Let's Encrypt) in a first pass, since that needs a real chunk of new x509 parsing (EKU, embedded-cert-chain verification) on top of an already-substantial feature. TLS 1.2 needs a new `CertificateStatus` message (type `22`, free); TLS 1.3's per-`CertificateEntry` extensions wire slot is already correctly framed (`build_certificate`/`decode_certificate`) but inert on both ends today (hardcoded empty on write, skipped-and-discarded on read) — real plumbing needed, not just a new extension type.
