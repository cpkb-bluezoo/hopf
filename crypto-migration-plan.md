...
**Phase 2 update, `KeyUpdate` (2026-09-10):** RFC 8446 §4.6.3/§7.2's post-handshake key ratchet is done — see the [`KeyUpdate`](#phase-2--tls-13-handshake-engine-quic-first) checklist entry. Unlike the last two P0 items looked at, this is a real, currently-published TLS 1.3 feature, not part of the fictional RFC 9846 extrapolation. TCP-TLS-1.3 only at the time (QUIC forbids it per RFC 9001 §4.6, enforced; DTLS 1.3's epoch-aware variant, RFC 9147 §5.8.4, has since been implemented - see the Phase 6 note below). Found and fixed a real latent bug along the way: `feed_handshake_data` was dropping all post-handshake bytes for the server role, not just gating KeyUpdate — server-role post-handshake message handling had never been exercised before this pass. Verified with real end-to-end application-data round-trips after a key rotation (not just secret-equality assertions), which is what caught that bug plus a missing event-forwarding override; full real `rustls` interop suite reconfirmed unaffected.
...
**Phase 6 update, DTLS 1.3 epoch-aware `KeyUpdate` (RFC 9147 §5.8.4):** `DtlsRecordEngine::request_key_update` plus automatic rotation at the AES-GCM confidentiality limit (both directions). The secret ratchet is the TLS 1.3 one under the `dtls13` label prefix; the epoch increments on every update. The receiver switches its read key at once and keeps the previous epoch's read keys until a record arrives under the new epoch (reordered datagrams still decrypt); the sender keeps writing under the old key until the peer's ACK for its `KeyUpdate` record arrives (RFC 9147 §8), retransmits the message until then, and the receiver re-ACKs a retransmission. Loopback-only - no external DTLS 1.3 peer has been run against this yet.
...
- [x] **RFC 9846 §1.4 TLS 1.2 deltas** — three of four done **(2026-09-10)**; the fourth is explicitly not applicable to this codebase's architecture, not merely undone:
  ...
  - [x] **`signature_algorithms_cert` parsed on receipt (#400)** - both ClientHello parsers now read the extension (`None` when absent or malformed), and both server engines refuse to send a certificate chain containing a signature scheme the client didn't list (`crypto::x509::chain_scheme_not_offered`; `handshake_failure`). Deliberately narrow: only applied when the client *explicitly* sent the extension (no fallback to `signature_algorithms`, whose legacy `rsa_pkcs1` semantics differ); self-signed certificates and schemes `x509` can't name are skipped, per RFC 8446 §4.2.3. RFC 8446 says a server SHOULD continue with its own chain if none fits; this aborts early instead, since the client would reject it anyway. Local chain *verification* is unchanged - the fixed `ACCEPTED_CERT_SIGNATURE_SCHEMES` allowlist is what we accept, whatever the peer offered. Not covered: the client-role counterpart (a server's `signature_algorithms_cert` in `CertificateRequest`, constraining the client certificate chain).
...
- [x] **UDP driver/listener wiring for DoDTLS/CoAPS consumers** (issue #404): `hopf-core::dtls::driver` — low-level, reactor-driven, per-peer-address sessions over one UDP socket, engine timers on the reactor's timer queue, DTLS 1.3 and 1.2, verified over real UDP loopback and against OpenSSL DTLS 1.2. Still deferred: the ACK handshake message (RFC 9147 §7); record sequence-number-confidentiality refinements beyond the fixed flag choice already shipped, and Connection IDs (RFC 9147 §4.2.3/§9); 0-RTT for DTLS (would reuse the existing TLS 1.3 ticket/PSK machinery, the same pattern `hopf-quic` used for QUIC 0-RTT); real interop against a DTLS-1.3-capable peer, once one exists.
...
- [x] **Cookie policy**: `HMAC-SHA256(cookie_secret, ClientHello.random)`, truncated to 16 bytes — real cryptographic binding to *this* handshake attempt; since #404 it can also be bound to the client's source address via `Dtls12Config::cookie_binding` (the UDP driver supplies `peer_cookie_binding(peer)`). DTLS 1.3's own cookie extension is still not sent. `Dtls12Config.require_cookie` defaults to **off**, since forcing the extra round trip for protection that isn't address-bound yet has a real cost and no real benefit.
...
- [ ] **Explicitly deferred**: (address-bound DTLS 1.2 cookies now shipped, see above); GnuTLS interop (OpenSSL was the minimum bar and is done; not attempted this pass).
...
- [x] **PQ (ML-DSA) signing: `CertificateVerify` + credentials (#405)** - closes the "verify only" limitation of the entry above. TLS 1.3 (and DTLS 1.3/QUIC, via the shared engine) now signs and verifies `CertificateVerify` with the pure ML-DSA schemes `mldsa44/65/87` (0x0904/5/6); they are appended to `SUPPORTED_SIGNATURE_SCHEMES`, so they are offered in `signature_algorithms` and `CertificateRequest`. The signing key's level is sniffed from its PKCS#8 `AlgorithmIdentifier` (`pkcs8_ml_dsa_level`) exactly as the classical types are. Works for both server and client (mTLS) credentials. `crypto::signature` gained `MlDsaLevel`, `MlDsaPrivateKey`, `ml_dsa_generate_pkcs8`, `ml_dsa_sign`, `ml_dsa_verify_spki`. **Credential format:** a standard PEM `PRIVATE KEY` (PKCS#8) holding an ML-DSA key - seed, expanded or both forms, whatever `aws-lc-rs` `PqdsaKeyPair::from_pkcs8` accepts - plus a PEM certificate whose key and signature are ML-DSA; loaded with the ordinary `acceptor_from_pem` / `connector_from_pem` helpers, no new API. New `crypto::x509::generate_ml_dsa_self_signed` issues a minimal self-signed identity (subjectAltName, no other extensions, valid 2026-2049) for development and tests, since `rcgen` still cannot. **Limitations:** TLS 1.2 has no ML-DSA codepoint, so a TLS 1.2 server holding an ML-DSA key refuses the handshake ("unsupported server signing key"); no hybrid PQ+classical certificate chains; no ML-DSA issuing of CA-signed chains (only self-signed; real chains come from a real CA, e.g. `openssl req -x509 -newkey ml-dsa-65`, OpenSSL 3.5+); ML-DSA public keys and signatures are large (ML-DSA-87 signature 4627 bytes), so pair with certificate compression (#395) for big chains. **Verified against real OpenSSL 3.6.4 both ways at ML-DSA-44/65/87**: hopf server with OpenSSL-generated PEM credentials (`s_client -verify_return_error` reports `Peer signature type: mldsaNN`, `Verification: OK`), and hopf client connecting to `s_server` (client Finished accepted, so OpenSSL's chain and `CertificateVerify` verified).
...
- [ ] **Optional, time-permitting after the above**: two independently real, already-finalized extensions worth scoping if this pass has room left, each needing its own dedicated scoping pass (RFC citation, wire format, and this codebase's own extension-parsing/build call sites) before implementation — not scoped in depth here, just confirmed as real and current rather than draft/speculative:
  - **Encrypted Client Hello (ECH)** — **implemented** (shared mode; HPKE built in-tree because `aws-lc-rs` exposes none; see `docs/conformance.html`). Original scoping note: now **RFC 9849** (published 2026-03-03; supersedes the long-running `draft-ietf-tls-esni` series, itself a redesign of the earlier ESNI mechanism). Encrypts the entire inner ClientHello (SNI, ALPN, etc.) inside an HPKE-wrapped payload alongside a cleartext "outer" ClientHello. Real, deployed (Cloudflare, Chrome, Firefox); OpenSSL added support following the RFC's publication. Needs HPKE (RFC 9180) as a prerequisite — check whether `aws-lc-rs` exposes it before scoping further.
  - [x] **TLS Certificate Compression** — **RFC 8879** (#395), Brotli only. `tls/handshake/cert_compression.rs` (bounded push decoder over the `brotli` crate, hard cap 256 KiB and exact declared-length match) + `HandshakeParser` streams a type-25 `CompressedCertificate` into it as bytes arrive and surfaces it as a plain `Certificate` (the transcript keeps the compressed wire form). Client offers `compress_certificate` (`HandshakeConfig::certificate_compression`, default on) and accepts; server compresses for a client that offered Brotli when it shrinks the message. TLS 1.3/DTLS 1.3/QUIC via the shared engine; TLS 1.2 excluded (RFC 8879 is 1.3-only). Not done: zlib/zstd, mTLS direction. Interop with real GnuTLS (`gnutls-cli`/`gnutls-serv --compress-cert=brotli`) in both directions; OpenSSL 3.6.4 here is built without brotli so could not be used.
...
- [RFC 9849 — TLS Encrypted Client Hello](https://www.rfc-editor.org/rfc/rfc9849.html) — implemented (shared mode); see `docs/conformance.html`
