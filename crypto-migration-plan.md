...
- [ ] **DTLS 1.3 real-peer interop** — closes the gap Phase 6 explicitly left open ("no RFC 9147 equivalent of RFC 8448's known-answer test vectors... no real peer either... none was available when this was written (checked OpenSSL, GnuTLS, and BoringSSL)"). Re-checked 2026-09-11: **wolfSSL** is the recommended peer — the first implementation of DTLS 1.3 (beta shortly after RFC 9147 published, April 2022), still under active RFC-compliance maintenance. **Now confirmed installed and ready on this machine (2026-09-11): wolfSSL 5.9.2 via MacPorts** (`port installed wolfssl` → `@5.9.2_0 (active)`). Verified directly, not assumed:
  - `/opt/local/include/wolfssl/options.h` has both `#define WOLFSSL_DTLS` and `#define WOLFSSL_DTLS13` — this build actually has DTLS 1.3 compiled in, not just the library in general.
  - `nm /opt/local/lib/libwolfssl.dylib` exports `wolfDTLSv1_3_client_method`/`_server_method` (and `_ex` variants) — the DTLS 1.3 method constructors are really present in the linkable library, confirmed by symbol name, not inferred from headers alone.
  - The port does **not** install prebuilt example binaries — only source under `/opt/local/share/doc/wolfssl/example/` (`client.c`, `server.c`, plus SCTP-DTLS variants; no standalone `client-dtls13.c`/`server-dtls13.c` the way the standalone `wolfssl-examples` GitHub repo has). `example/client.c`/`example/server.c` **do** support DTLS 1.3 via CLI flags, confirmed by grepping their own `-u`/`-v` help text: `-u` selects UDP DTLS, `-v 4` selects DTLS 1.3 (`#ifdef WOLFSSL_DTLS13` / `#endif` guards around the `-v 4` help text in both files). These need a one-time local compile against the installed lib before use as the interop harness's subprocess binaries — `wolfssl-config --cflags --libs` reports `-I/opt/local/include -L/opt/local/lib -lwolfssl -lnetwork -lpthread`, so e.g. `cc example/client.c -o /tmp/wolfssl-dtls-client $(wolfssl-config --cflags --libs)` is the expected shape (confirm exact flags/any needed `test.h` support objects when actually building — wolfSSL's own example sources sometimes pull in shared test-utility code from the same `examples/` tree that isn't part of this trimmed MacPorts doc bundle; check `wolfssl-examples` on GitHub or the full wolfSSL source tarball for the companion files if a bare compile fails).
  - Same shape as this project's existing subprocess-over-real-UDP interop harness (`dtls12::interop_tests`, OpenSSL-based) — reuse that harness's structure, swapping the OpenSSL `s_client -dtls1_2`/`s_server -dtls1_2` subprocess invocations for the compiled wolfSSL `client -u -v 4`/`server -u -v 4` ones.
  - Ruled out: GnuTLS (no DTLS 1.3 in any released version; open feature request since 2020), mbedTLS (explicitly documented as unsupported), s2n-tls (no DTLS support at all, any version), NSS (only stale pre-RFC draft-era support, no scriptable CLI), pion/dtls (DTLS 1.3 still in-progress on `main`, unreleased). BoringSSL has real DTLS 1.3 support but no stable release and no CLI tool comparable to `s_client`/`s_server` (its own interop model needs a custom Go-runner-plus-C++-shim harness) — disproportionate integration cost versus wolfSSL for the same coverage.
  - **Watch item, not this pass's dependency**: OpenSSL landed DTLS 1.3 in `4.1.0-alpha1` (2026-09-09) with stable `4.1` expected ~October 2026 — worth adding as a *second* peer once stable, since the existing OpenSSL-based DTLS 1.2 harness would need only new flags (`-dtls1_3`), not new tooling; not usable as the primary target today (alpha, explicitly "not for production").
  - **Tests:** mirror `dtls12::interop_tests`'s shape (real subprocess + real UDP socket, both directions — hopf client vs. wolfSSL server, wolfSSL client vs. hopf server) rather than the in-process `rustls`-style harness, since wolfSSL has no Rust bindings usable in-process here. Expect this to surface real bugs the way every other real-interop pass this project has run has (Phase 4: 5 bugs; Phase 5: 1; Phase 6 DTLS 1.2: 5) — RFC 9147's wire shapes in this codebase are explicitly flagged as "cross-checked against the RFC text but not proven against a real peer" today.
